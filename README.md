...
* [ ] 2-opt 경로 개선, 이동수단별 시간 추정과 10분 단위 올림, 기하 중앙값·메도이드 구현 및 순수 알고리즘 회귀·성능 검증
...
* [ ] **완료 일정 Aggregate와 API**: TravelPlan·Day·Item·PlanPlace 모델과 migration, 계산 완료 후 짧은 트랜잭션 저장, 목록·상세·제한 편집·삭제·공유 조회를 구현한다. 완료/공유 조회에는 외부 API를 호출하지 않고 저장 데이터와 Kakao 링크만 사용한다.
* [x] Routy app shell·8단계 제작 화면 골격, 회원가입·로그인, 지역 직접 검색과 AI 지역 추천 선택을 백엔드에 연결하고 서버 TTL 기반 메모리 인증 상태를 검증
...
세부 작업 순서와 완료 판정은 [`docs/11-command-roadmap.md`](./docs/11-command-roadmap.md)를 따릅니다. 현재 개발 기반, 인증·호출 한도, 국내 지역 기준·검색, 지역·메뉴 AI API, 인증·지역 선택 화면 연결이 구현·검증됐습니다. 순수 경로 알고리즘은 Haversine 거리와 Nearest Neighbor까지 완료됐으며, 다음 백엔드 작업은 2-opt 경로 개선입니다. 이후 이동 시간 추정과 순수 알고리즘 검증을 마친 뒤 외부 Client, 장소 선택, 실제 경로 검증, 일정·추천, 완료 일정 순서로 핵심 흐름을 완성합니다.
