...
`W1-01A`에서 W1-00 골격에 회원가입·로그인 API adapter, 메모리 인증·만료·로그아웃과 보호 화면 진입을 연결했다. Node 순수 테스트 20개와 Chromium 브라우저 테스트 9개 시나리오(상위 테스트 포함 총 Node 30개), 전체 Gradle 테스트 68개와 `git diff --check`가 통과했다. 브라우저는 격리 HTTP fake를 사용했으며 실제 Spring 서버와 브라우저를 연결한 end-to-end smoke는 수행하지 않았다. 다음 시작 작업은 국내 지역 기준 데이터의 공식 출처와 갱신 절차를 확정하는 `G1-01`이다. `W1-01B`는 G1·A1-04 선행 조건을 충족한 뒤 진행한다. 작업을 시작할 때 현재 Git 상태를 확인하고 하나의 작업 ID와 Change Envelope를 정한다.
