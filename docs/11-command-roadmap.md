...
`G1-02`에서 행정표준코드관리시스템 법정동 코드와 국토교통부 행정구역도 WFS 경계를 대조해 정적 `regions.json` 246개를 구축했다. 최종 선택 지역 161개와 장소 검색 필터 76개를 분리하고 읍·면·동·리와 해외를 제외했으며, 광주의 사용자 표시명과 `전남광주통합특별시` 주소 경계, 도 산하 분구시의 시 단위 선택 계약을 반영했다. 원천 코드 집합·역할·부모·주소 경계·좌표 범위·대표점 경계 포함을 독립 검증했고, Docker 소켓을 연결한 격리 Gradle/JDK 환경에서 Testcontainers MySQL 8.4를 포함한 전체 Gradle 테스트와 `git diff --check`가 통과했다. 다음 시작 작업은 Region 값 객체·Loader·시작 검증을 구현하는 `G1-03`이다. `W1-01B`는 G1·A1-04 선행 조건을 충족한 뒤 진행한다. 작업을 시작할 때 현재 Git 상태를 확인하고 하나의 작업 ID와 Change Envelope를 정한다.
