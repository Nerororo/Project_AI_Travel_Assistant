...
| G1-05A | OpenAI 공통 공식 계약 감사 | 모델·구조화 출력·endpoint·인증·timeout·재시도·비용·전체 예산 확정 |
| G1-06 | 승인된 계약에 따른 OpenAI 실제 Client 구현 | G1-05A 결정 준수, 자동 테스트는 Fake, 지역·메뉴용 HTTP 구현 중복 없음 |
...
| C1-06 | 카카오 실제 Client 구현 전 공식 계약 감사 | Local·자동차·대중교통 endpoint·인증·쿼터·요금·정책 기록 |

실제 카카오 HTTP 호출과 좌표를 받는 공개 endpoint는 완료된 K0-02A를 선행 조건으로 P1·R2에서 구현한다.

## 8. P1 — 카카오 장소 제작 흐름

선행 조건: K0-02A와 C1-06 완료.
...
| W1-01B | U1·G1 | 지역 직접 검색·AI 추천 UI 연결 | 국내 허용 목록, 직접 검색과 정확히 3개인 AI 후보 선택 |
| W1-02 | P1 | 이동수단·관광지·숙소 선택 UI 연결 | P1-06 상태 모듈 사용, 관광지 구·군 필터 선택·명시적 해제, 지도 이동 재검색 시 필터 유지, 목록·지도·마커, 빈 이름 입력, 체류 조정, 지역 변경 확인과 상태 폐기 |
