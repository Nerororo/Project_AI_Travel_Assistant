...
`G1-02`에서 행정표준코드관리시스템 법정동 코드와 국토교통부 행정구역도 WFS 경계를 대조해 정적 `regions.json` 246개를 구축했다. 최종 선택 지역 161개와 장소 검색 필터 76개를 분리하고 읍·면·동·리와 해외를 제외했으며, 광주의 사용자 표시명과 `전남광주통합특별시` 주소 경계, 도 산하 분구시의 시 단위 선택 계약을 반영했다. `G1-03`에서 Region 불변 값 객체와 Loader·메모리 Catalog를 구현하고 데이터·역할·부모·주소 경계·좌표를 시작 시 검증하도록 했다. `G1-04`에서 공식 이름·짧은 이름·별칭의 결정적 검색 Service와 인증 `GET /api/regions`를 연결했다. `G1-05`에서 161개 최종 선택 지역, 76개 검색 필터, 9개 상위 탐색 항목과 광주·세종·도 산하 분구시의 경계 계약을 회귀 테스트로 보강하고, Docker 소켓을 연결한 격리 Gradle/JDK 환경에서 Testcontainers MySQL 8.4를 포함한 전체 91개 테스트를 통과해 G1 단계를 완료했다. 다음 시작 작업은 지역 추천 요청·응답 DTO와 validation을 정의하는 `A1-01`이다. `W1-01B`는 A1-04 선행 조건도 충족한 뒤 진행한다. 작업을 시작할 때 현재 Git 상태를 확인하고 하나의 작업 ID와 Change Envelope를 정한다.
