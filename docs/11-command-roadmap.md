...
인증·호출 한도, 국내 지역 검색, 지역·메뉴 AI API, 지역 선택 화면과 순수 경로 알고리즘까지 구현·검증했다. 순수 경로는 고정 출발·도착 경계, Haversine, Nearest Neighbor, 2-opt, 이동수단별 시간 추정, 기하 중앙값과 메도이드를 포함하며 최대 하루 입력과 7일 조합 회귀를 통과했다.

다음 시작 작업은 `C1-01`이다. 외부 장소 Client의 요청·응답 DTO와 오류 계약을 설계하고 외부 필드를 내부 모델과 분리한다.
