...
→ P1 장소
→ R2 독립 경로 기반
→ S1-01~05 추정 일정·숙소 검색 → W1-02 가능 → W1-02A 가능
→ S1-06~07 음식점 추천·점검 → W1-03·W1-03A 가능
...
| G1-06 | 모든 최종 지역의 공식 경계 기반 `searchBounds` 생성 | WGS84 최소·최대 좌표, 논리·분구 지역 합집합, 전체 경계 포함, Loader 검증 |
...
| C1-05A | 카카오 공식 흐름과 Client 계약 정렬 | Local 도시 전체 공간 검색, 자동차 결과 코드, 대중교통 후보·시간 의미·상태 매핑 확정 |
| C1-05B | Local radius·rect 요청 계약 구현 | 두 공간 입력의 정확히 하나만 허용하고 page·호출량 단위를 보존하는 DTO·Fake 계약 테스트 |

실제 카카오 HTTP 호출과 좌표를 받는 공개 endpoint는 완료된 K0-02A를 선행 조건으로 P1·R2에서 구현한다.

## 9. P1 — 카카오 장소 제작 흐름

선행 조건: K0-02A, C1-05A~B와 G1-06 완료.
...
| W1-02 | P1·S1-05·W1-00 | 이동수단·관광지·숙소 선택 UI 연결 | P1-06 상태 모듈 사용, 관광지 구·군 필터 선택·명시적 해제, 지도 이동 재검색 시 필터 유지, 목록·지도·마커, 빈 이름 입력, 관광지 체류 30~480분·10분 조정과 유형 미노출, 숙소 거리 점수·자동 순위 없는 직접 선택, 지역 변경 확인과 상태 폐기 |
...
| W1-03B | T1-06A·W1-03 | 경로 없음 구간 표시와 재시도 | 422 뒤 작성 상태 유지, `date + moveOrder` 앞뒤 항목 강조, 불일치 시 날짜 단위 오류, 사용자 조정 뒤 전체 재검증 |
...
`R2-06`: 경로의 timeout·연결 실패·일시적 5xx를 한 번 재시도하고, 재실패하면 부분 구간이 아닌 일정 전체를 Haversine 예상시간으로 fallback하며 정상 경로 없음은 422 계약으로 보존한다.
