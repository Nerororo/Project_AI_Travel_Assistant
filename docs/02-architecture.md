...
회원가입, 비밀번호 해시, 로그인, JWT, 인증 사용자 식별, 일정 소유권과 사용자·서비스 호출 카운터 및 `requestId` 실행 상태 저장소를 담당한다. 다른 도메인은 user의 공개 Service·DTO로 호출량을 확보하고 중복 상태를 확인한다. 일정은 처음부터 인증 사용자 소유로 저장한다.

### region

`regions.json`을 시작 시 검증·적재하고 서울특별시·광역시·세종특별자치시는 자체를, 도·특별자치도는 그 아래 시·군을 최종 선택 항목으로 제공한다. 특별시·광역시 아래 구·군은 장소 검색 범위를 좁히는 선택 필터이며 최종 일정 지역으로 사용할 수 없다. 읍·면·동은 제외한다. 지역 ID, 이름, 상위 지역, 선택 가능 여부와 대표 좌표를 제공하며 AI가 선택할 허용 목록도 소유한다. 완료 일정에는 최종 지역 하나의 `regionId`와 당시 표시 이름을 저장한다.
...
계층별 테스트 책임은 `docs/08-test-strategy.md`, 완료 판정은 `docs/10-definition-of-done.md`를 따른다.

## 11. 확인된 카카오 데이터 수명 계약

세부 수명 조건은 `docs/06-decisions.md`의 ADR-028, 운영 검증과 정책 변경 대응은 `docs/09-operations.md` 9절을 단일 기준으로 사용한다. 아키텍처에서는 정적 지역 데이터, 인증, 순수 알고리즘, Client 인터페이스와 Fake, 저장 모델을 좌표의 수명과 저장 금지 계약을 침범하지 않도록 분리한다.
