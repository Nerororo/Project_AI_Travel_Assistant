...
장소 검색과 AI는 오류를 임의 데이터로 대체하지 않는다. Kakao Local 장소 검색은 연결 timeout 2초, 재시도를 포함한 전체 요청 시간 예산 6초를 사용한다. 인증·권한·잘못된 요청·rate limit과 손상 응답은 재시도하지 않고, 연결 실패·timeout·5xx만 전체 예산 안에서 최대 한 번 재시도한다. timeout과 전체 요청 시간 예산은 제공자 보장값이 아니라 검색 화면의 응답성을 위한 Routy 운영값이다.
