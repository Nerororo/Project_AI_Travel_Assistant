...
이 문서는 운영 환경을 추가할 때 따라야 할 최소 기준이다. 현재는 설계 문서이며, Actuator·배포 플랫폼·지도 API 공급자가 확정되기 전에는 설정 파일이나 dependency를 추가하지 않는다.
...
- 지도 API 공급자가 정해지기 전에는 공급자별 설정 키를 문서나 코드에 확정하지 않는다. 확정 시 API 키, base URL, timeout, 호출 한도를 같은 방식으로 설정 분리한다.
...
| 지도 API | dependency | 장애 시 장소 탐색/갱신 기능만 실패하며 원인을 구분 |
...
- 지도 API도 공급자 확정 후 timeout, 재시도 대상 상태 코드, rate limit 대응을 명시한다.
