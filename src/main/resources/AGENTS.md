...
- `data/regions.json`은 출처가 확인된 최종 선택 가능한 서울·광역시·세종, 상위 도·특별자치도와 그 아래 최종 시·군, 특별시·광역시의 검색 필터용 구·군 기준 데이터만 포함하고 카카오 검색 결과를 복제하지 않는다.
- `static`은 브라우저 공개 자산이므로 서버 비밀과 내부 진단 정보를 포함하지 않는다.
- migration은 `docs/03-database.md` 계약을 따르고 적용된 versioned 파일을 수정하지 않는다.
- `api_usage_counters`와 `request_executions` migration에는 좌표·결과 리소스 ID·외부 payload·response 컬럼을 추가하지 않는다. `request_executions`는 처리 상태와 만료 시각만 저장한다.
