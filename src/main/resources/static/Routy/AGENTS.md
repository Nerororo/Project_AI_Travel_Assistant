...
현재 파일은 정적 시안이며 실제 API·DB·인증·카카오 연동 완료로 간주하지 않는다. 시각 UI와 API 화면 연결은 로드맵의 `W1` 단계에서 진행한다. 예외적으로 `P1-06`에서 승인한 비시각 브라우저 메모리 상태 모듈·장소 임시 상태 전달 adapter와 직접 대응 순수 상태 테스트는 P1 경계에서 구현할 수 있으며, 이때 화면 구조·표현·지도·마커를 변경하지 않는다.
...
- `P1-06`의 Allowed는 사전 지정한 비시각 메모리 상태 파일·adapter·순수 상태 테스트로 한정하며 W1 화면 구현 권한으로 확대하지 않는다.
